...
class RMVPE:
    ...
    def infer_from_audio_batch(self, audios, sample_rate=16000, thred=0.03, use_viterbi=False):
        ...
        # breakpoint()
        f0, cent = self.decode(hiddens, thred=thred, use_viterbi=use_viterbi)
        f0s = []
        cents = []
        for i in range(f0.shape[0]):
            f = f0[i, :sizes[i]]
            cent_i = cent[i, :sizes[i]]
            cents.append(cent_i)
            f0s.append(f)
    ...
    def get_pitch_batch(self, waveforms, sample_rate, hop_size, lengths, interp_uv=False, fmin=50, fmax=1000):
        ...
        # breakpoint()
        f0s, indexs, cents = self.infer_from_audio_batch(waveforms, sample_rate=sample_rate)
        f0s_res, uvs_res = [], []
