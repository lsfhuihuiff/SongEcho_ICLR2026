...
def optimized_batch_by_size(
        indices, num_tokens_fn, max_tokens=None, max_sentences=None,
        required_batch_size_multiple=1
):
    """
    优化版本的 batch_by_size，保留原始逻辑，处理 (idx, size) 元组并返回索引列表。

    Args:
        indices (List[Tuple[int, int]]): 包含 (index, size) 的列表
        num_tokens_fn (callable): 返回给定索引的 token 数的函数
        max_tokens (int, optional): 每个批次的最大 token 数
        max_sentences (int, optional): 每个批次的最大句子数
        required_batch_size_multiple (int, optional): 批次大小需为 N 的倍数

    Returns:
        List[List[int]]: 按 token 数分组的索引批次列表
    """
    ...
    # 按 size 降序排序以优化批次填充
    sorted_indices = sorted(indices, key=num_tokens_fn, reverse=True)
    ...
    for idx, size in sorted_indices:
        sample_lens.append(size)
        sample_len = max(sample_len, size)

        # 使用原始逻辑计算 num_tokens
        num_tokens = (len(batch) + 1) * sample_len

        if _is_batch_full(batch, num_tokens, max_tokens, max_sentences):
            # 确保批次大小为 required_batch_size_multiple 的倍数
            mod_len = max(
                bsz_mult * (len(batch) // bsz_mult),
                len(batch) % bsz_mult,
            )
            batches.append([idx for idx, _ in batch[:mod_len]])  # 直接返回索引
            batch = batch[mod_len:]
    ...
    if batch:
        ...
        batches.append([idx for idx, _ in batch[:mod_len]])
