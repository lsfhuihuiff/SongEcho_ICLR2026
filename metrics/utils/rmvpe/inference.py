...
from .audio_utils.audio.pitch_utils import interp_f0, resample_align_curve
...
class RMVPE:
    ...
    def infer_from_audio(self, audio, sample_rate=16000, thred=0.03, use_viterbi=False):
        ...
        # breakpoint()
        size = math.ceil((audio[0].shape[0]) / self.hop_length)
        if sample_rate == 16000:
            ...
            size = math.ceil(size * (16000 / sample_rate))
    ...
    def infer_from_audio_batch(self, audios, sample_rate=16000, thred=0.03, use_viterbi=False):
        ...
        sizes = [math.ceil((audio.shape[0]) / self.hop_length) for audio in audios]
        ...
        if sample_rate == 16000:
            ...
            sizes = [math.ceil(size * (16000 / sample_rate)) for size in sizes]
    ...
    def get_pitch_batch(self, waveforms, sample_rate, hop_size, lengths, interp_uv=False, fmin=50, fmax=1000):
        ...
        f0s, indexs = self.infer_from_audio_batch(waveforms, sample_rate=sample_rate)
        f0s_res, uvs_res = [], []
