...
class CustomerAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ################mark: add ip_attn
        if hasattr(attn, "melody_to_k"):
            # print("begin")
            melody_k = attn.melody_to_k(encoder_hidden_states_con)
            melody_v = attn.melody_to_v(encoder_hidden_states_con)
            melody_k = melody_k.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            melody_v = melody_v.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)

            melody_k_length = melody_k.shape[2]

            rotary_freqs_cis_melody = attn.rotary_emb(
                melody_k, seq_len=melody_k_length
            )
            melody_k = self.apply_rotary_emb(melody_k, rotary_freqs_cis_melody)
            melody_v = self.apply_rotary_emb(melody_v, rotary_freqs_cis_melody)

            melody_hidden_states = F.scaled_dot_product_attention(
                query, melody_k, melody_v, attn_mask=None, dropout_p=0.0, is_causal=False
            )
            # print(query.shape, melody_k.shape, melody_v.shape, melody_hidden_states.shape)
            melody_hidden_states = melody_hidden_states.transpose(1, 2).reshape(
                batch_size, -1, attn.heads * head_dim
            )
            melody_hidden_states = melody_hidden_states.to(query.dtype)
            melody_hidden_states = melody_hidden_states.transpose(1, 2)
            melody_hidden_states = attn.conv_out(melody_hidden_states)
            melody_hidden_states = melody_hidden_states.transpose(1, 2)
            hidden_states = hidden_states + attn.scale * melody_hidden_states
        ################end mark
