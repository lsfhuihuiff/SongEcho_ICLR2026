...
class Text2MusicDataset(Dataset):
    ...
    def get_speaker_emb_file(self, speaker_emb_path):
        ...
        try:
            if speaker_emb_path.endswith(".npy"):
                data = np.load(speaker_emb_path)
                data = torch.from_numpy(data).float()
                data = data.squeeze(0)
            elif speaker_emb_path.endswith(".pt") or speaker_emb_path.endswith(".pth"):
    ...
    def get_audio(self, item):
        ...
        # print(f"duration: {audio.shape[-1] / 48000:.2f} seconds")
    ...
    def process(self, item):
        ...
        if self.load_melody:
            filename = item["filename"]
            melody_path = filename.replace("/audio", "/audio_F0").replace(".mp3", ".pt")
            melody = torch.load(melody_path, weights_only=False, map_location="cpu")["f0"]
        ...
        # print("speaker_emb_path:", speaker_emb_path)
        if speaker_emb_path:
            speaker_emb = self.get_speaker_emb_file(speaker_emb_path)
        # print("shape:", None if speaker_emb is None else speaker_emb.shape)
        # if not speaker_emb_path:
        #     speaker_emb = self.get_speaker_emb_file(speaker_emb_path)

        if speaker_emb is None:
            speaker_emb = torch.zeros(512)
        # print(type(speaker_emb))
        # print(speaker_emb.device)
        ...
        example = {
            ...
            "melody": melody if self.load_melody else None,
        }
    ...
    def collate_fn(self, batch):
        ...
        for k, v in batch.items():
            ...
            elif k in ["src_wavs", "target_wavs", "vocal_wavs", "melodys"]:
                # print(k, [seq.shape for seq in v])
                if len(v) == 0:
                    continue
